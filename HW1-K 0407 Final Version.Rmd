...
\
  若該名學生第一次就滿足可修個經的條件，則其"$個經PR_i$"受"$個經PR_{1i}$"影響；若該名學生並未第一次就滿足可修個經的條件，則其"$個經PR_i$"受到"$個經PR_{0i}$：經原或微積分修一次以上"以及未來通過擋修門檻可修個經時"$個經PR_{1i}$"的影響。
...
# 看到圖後，可猜測若以"可修個體"為解釋變數、"個經PR"為被解釋變數，所跑出來的OLS模型可能會產生偏誤。
```
...
summary(model1)    
#可修個體的係數為0.07743(>0)，表示可修個體對個經PR有正向影響。
...
&=\frac{n_1\bar{y_1}-n_1(n_0\bar{y_0}+n_1\bar{y_1})/n}{n_1n_0/n} \\

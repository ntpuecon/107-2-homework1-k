...
(3)同理，微積分成績好，各經成績也不一定好，但因在修習個經的前提條件中：只要求有修微積分且微積分成績不為0，即可修個經，因此推測微積分修得好不好，雖然也有可能影響各經成績，但與經原的影響力相比應較小。

## 1.2 效應結構
接續上題，課堂上的treatment dummy即為這裡的**可修個體**dummy，請寫下對應課堂效應結構$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})T_i$之對應寫法（以這裡的文字符號表示）。

ans:
     $個經PR_i=經原不及格成績_i+(經原及格成績_i-經原不及格成績_i)T_i$

  $個經PR_i=不可修個經_i+(可修個經_i-不可修個經_i)T_i$


??我想：經原成績不及格的話，就不能修個經，就不會有個經PR，所以此條式子還是怪怪
...
請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)____(上升/下降) (2)____百分點，其值比未控制**經原PR**時還 (3)____(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)____(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：

  1. **經原PR**與**個經PR**有(5)____(正向/反向)關連
  2. **可修個體**=1的族群，其**經原PR**平均較(6)____(高/低)
  
故若不控制**經原PR**，因**可修個體**=1與=0兩群人也會分別帶有 **經原PR**  (7)____(高與低/低與高)特質，所以**可修個體**=1的人其**個經PR**高，有部份是因其**經原PR**也較(8)____(高/低)所至。
...
$V(\hat{\beta}|X)=E{[(\hat{\beta})-E(\hat{\beta})|X][(\hat{\beta})-E(\hat{\beta})|X]'}=E[(\hat{\beta}-\beta)(\hat{\beta}-\beta)']=(X'X^{-1}X'\epsilon)(X'X^{-1}X'\epsilon)'=(X'X)^{-1}X'E(\epsilon\epsilon') X(X'X)=(X'X)^{-1}X'\Sigma X(X'X)$

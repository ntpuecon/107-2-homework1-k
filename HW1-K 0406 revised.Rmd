...
coeftest(model_na, vcov.= vcovHC, type="HC1")
coeftest(model_na, vcov. = vcovHC, type="HC1")-> model_na_coeftest

stargazer(model_na_coeftest,
          se=list(model_na_coeftest,"std. error"),
          type = "html",
          align = TRUE)
...
NOTE:你去看hw1Data裡面有可修個經=0的人，卻有個經PR（不是NA）。有問題的話我們可以再討論

(2)經原成績好，個經成績就一定好嗎? 不一定吧，但若想修個經，就一定要成功修習通過經原。
因此猜測經原修得好不好，可能會影響個經修的好不好。而其餘影響個經成績的可能因素有：作業有沒有交、考試當天有沒有不小心睡過頭、上課認不認真、平時有無課後複習得習慣等等...
(3)同理，微積分成績好，個經成績也不一定好，但因在修習個經的前提條件中：只要求有修微積分且微積分成績不為0，即可修個經，因此推測微積分修得好不好，雖然也有可能影響個經成績，但與經原的影響力相比應較小。

----------------

* 我們應該要思考，可以修個經的人若不可修個經，則其個經PR應與原本就不可修個經的人相同（需要站在同一個立足點上才能做比較，即假設兩種人的"學習能力相同"）。
然而1.3之估計結果很可能不正確的原因即是立足點的不同所造成。我們想要探討擋修制度是否會讓學生在達到一定基礎後，對後續的學習有較佳的效果。但是這裡卻是以可不可修個體去對個經PR跑迴歸來驗證，這可能會造成偏誤。首先，可修個體者（通過經原，微積分不為0分）我們可能會認為其學習能力較強，理應會有較高的個經PR。但若一名學習能力較弱的學生經原修了三次才通過，但是以經原PR99通過這門課，我們會有很高的機率相信他的個經PR表現也會很好（基礎雖然不好，但若修了三次、長期都埋首在經濟學中，經濟實力應該可以顯著提升）。所以我們若只考慮了門檻（可不可修個經），可能會造成偏誤；換一個角度想，若一名學習能力很好的學生，且其可修個經，但若他當時是以經原60分通過這門課（從小學習能力好，再打基礎的時候可能稍微打混摸魚，但還是修過經原），我們也會有很高的機率相信他的個經PR表現會不好。
因此我們應該將經原PR納入模型中，再去探討擋修制度是否會對學生的學習成效造成正面影響。
...
* 同上題，在進行迴歸估計時，除了變數的選擇，我們亦需要將立足點考慮進去。我們認為可修個體與否會影響到個經PR，但必須控制在立足點相同的前提之下。若我們在進行估計時，將有相同背景（經原PR相同，我們會認為學習能力可能差不多）的人控制住，即可用"可修個體"與否來探討擋修制度對個經PR的影響，專心探討我們真正在意的變數（"可修個體"對"個經PR"的解釋力）。
...
\
 $$E[E(\hat{\beta}_1|x_i)]=E({\beta}_1)=\beta_1$$
